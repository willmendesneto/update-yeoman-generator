...
const normalizeEJSTemplateDelimiters = (filename, ejsDelimiterOpen, ejsDelimiterClose) =>
  filename.replace(ejsDelimiterOpen, DEFAULT_EJS_OPEN).replace(ejsDelimiterClose, DEFAULT_EJS_CLOSE);

const toPascalCase = flow(
  camelCase,
  upperFirst,
);
...
module.exports = {
  ...
  isAnEJSTemplate: (filename, props, ejsDelimiterOpen, ejsDelimiterClose) => {
    const ejsFilename = normalizeEJSTemplateDelimiters(filename, ejsDelimiterOpen, ejsDelimiterClose);
