...
module.exports = (
  ...
  const filePatchHandler = (patch, fileOriginal, fileB, localFile) => {
    switch (patch.status) {
      case CONFLICT_STATUS_REMOVED:
        printMessage(`${chalk.white.bgRed(patch.status)}: ${localFile}`);
        if (fs.existsSync(localFile)) {
          removeFilesInDirectory(localFile);
        }
        break;
      case CONFLICT_STATUS_ADDED:
        printMessage(`${chalk.black.bgGreen(patch.status)}: ${localFile}`);
      ...
      case CONFLICT_STATUS_MODIFIED:
        printMessage(`${chalk.gray.bgYellow(patch.status)}: ${localFile}`);
        mergeFileIfExists(localFile, fileOriginal, fileB);
        break;
      case CONFLICT_STATUS_RENAMED: {
        printMessage(`${chalk.gray.bgCyan(patch.status)}: ${localFile}`);
      ...
      default:
        printMessage(chalk.red(`Unhandled case: File ${patch.status}: ${localFile}`));
    }
  };
  ...
  const applyingFileChanges = ([firstFileContent, secondFileContent]) => {
    ...
    printMessage(chalk.yellow(`File modified: ${localFile}`));
