...
module.exports = (generator, templatePrefix, ejsOpen, ejsClose) => {
  ...
  const updateGeneratorVersion = (newGeneratorVersion) => {
    ...
    printMessage(chalk.inverse(' INFO ') +
      `Updating generator ${generator} from: v${oldGeneratorVersion} to v${newGeneratorVersion}`);
  ...
  return getPackageJsonData(generator, token)
    ...
    .then(({ version }) => updateGeneratorVersion(version))
    .then(() => githubDiff(
      generator,
